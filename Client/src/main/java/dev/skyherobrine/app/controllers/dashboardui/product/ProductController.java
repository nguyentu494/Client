...
public class ProductController implements ActionListener, MouseListener, KeyListener, TableActionEvent, TableModelListener {
    ...
    public ProductController(QuanLySanPham sanPhamUI) {
        try {
            ...
            loaiSanPhamDAO = instance.getLoaiSanPhamDAO();
            thuongHieuDAO = instance.getThuongHieuDAO();
            danhMucSanPhamDAO = instance.getDanhMucSanPhamDAO();
            chiTietPhienBanSanPhamDAO = instance.getChiTietPhienBanSanPhamDAO();
            thueDAO = instance.getThueDAO();
            loadDsSanPham();
...
//                                        MatrixToImageWriter.writeToPath(matrix, "jpg", Paths.get(path));
                                ...
                                System.out.println(trangThaiNutXoa);
                            } else {
        ...
        if (e.getSource().equals(sanPhamUI.getBtnThemPBSP())) {
            ...
            if (trangThaiNutThem == 0) {
                ...
                sanPhamUI.getBtnDialogThemXuong().setVisible(true);
                sp = layDataThem();
                if (sp == null) {
                    return;
                }
