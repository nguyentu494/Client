...
public class KhachHangController implements MouseListener, ActionListener, PropertyChangeListener, KeyListener {
    ...
    private static KhachHangDAO<KhachHang> khachHangDAO;
    ...
    public KhachHangController(FrmKhachHang khachHangUI) {
        ...
        try {
            ...
            khachHangDAO = instance.getKhachHangDAO();
            loaddsKhachHang();
        ...
        if(op.equals(khachHangUI.getCbTkGioiTinh())){
            if(khachHangUI.getjDateChooserTkNgaySinh().getDate() == null){
                if(!khachHangUI.getCbTkGioiTinh().getSelectedItem().equals("--Giới tính--")){
                    ...
                    conditions.put("gioiTinh", khachHangUI.getCbTkGioiTinh().getSelectedItem().toString().equals("NAM") ? "true" : "false");
                    try {
...
//    }
    ...
    public void mouseClicked(MouseEvent event) {
        if(trangThaiNutThemKH==1){
            ...
            KhachHang khHienThuc = new KhachHang();
            try {
                khHienThuc = instance.getKhachHangDAO().timKiem(ma);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        ...
        if ("date".equals(propertyName) && source instanceof JDateChooser) {
            ...
            if ("NgaySinhTK".equals(dateChooserName)) {
                if(!khachHangUI.getCbTkGioiTinh().getSelectedItem().equals("--Giới tính--")){
                    ...
                    Map<String, Object> conditions = new HashMap<>();
                    String date = convertDateToString(khachHangUI.getjDateChooserTkNgaySinh().getDate());
                    conditions.put("ngaySinh", "%"+date+"%");
                    try {
                        dsLoc = khachHangDAO.timKiem(conditions);
                    } catch (Exception ex) {
                        throw new RuntimeException(ex);
