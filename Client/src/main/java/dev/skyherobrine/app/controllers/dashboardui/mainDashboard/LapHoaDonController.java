...
public class LapHoaDonController implements KeyListener, Runnable, ThreadFactory, ActionListener, TableModelListener, TableActionEvent1, TableActionEvent {
    ...
    private final ChiTietHoaDonDAO<ChiTietHoaDon> chiTietHoaDonDAO;
    private DefaultListModel<String> listModel;
    private final ChiTietPhienBanSanPhamDAO<ChiTietPhienBanSanPham> chiTietPhienBanSanPhamDAO;
    private WebcamPanel pnCam = null;
    private Webcam webcam = null;
    private final Executor executor = Executors.newSingleThreadExecutor(this);
    private final SanPhamDAO<SanPham> sanPhamDAO;
    private final NhanVienDAO<NhanVien> nhanVienDAO;
    private final KhachHangDAO<KhachHang> khachHangDAO;
    private final HoaDonDAO<HoaDon> hoaDonDAO;
    ...
    public LapHoaDonController(LapHoaDon lapHoaDon) {
        ...
        try {
            ...
            this.nhanVienDAO = instance.getNhanVienDAO();
            this.khachHangDAO = instance.getKhachHangDAO();
...
//       lapHoaDon.getPnCamera().add(avb);
    ...
    public void searchSuggestKH(String textKH) {
        ...
        try {
            List<Map<String, Object>> listKH = khachHangDAO.timKiem(conditions, false, "soDienThoai", "hoTen");
            if (listKH.isEmpty()) {
                lapHoaDon.getMenuKhachHang().setVisible(false);
            } else {
...
//                System.out.println(conditions.size());
                ...
                if(lapHoaDon.getjCheckBox1().isSelected()){
                    ...
                    kh = khachHangDAO.timKiem(conditions).get(0);
                }
                if (kh == null) {
                    kh = new KhachHang("KH-00000000-000", "Khách vãng lai", "0000000000", true, LocalDate.now(), 0);
                    khachHangDAO.them(kh);
                }
                HoaDon hd = new HoaDon(maHD, nl, nhanVienDAO.timKiem(maNV), kh, new BigDecimal(tienKHTra), null);
    ...
    public boolean themSP(String maPBSP) {
        ...
        try {
            ...
            SanPham sp = sanPhamDAO.timKiem(maSP);
            Map<String, Object> conditions = new HashMap<>();
...
//                    }
    ...
    public void tableChanged(TableModelEvent e) {
        if (e.getColumn() == 4) {
            ...
            double tta = 0;
            try {
                pbsp = chiTietPhienBanSanPhamDAO.timKiem(tmGioHang.getValueAt(row, 1).toString());
            } catch (Exception ex) {
                throw new RuntimeException(ex);
            }
