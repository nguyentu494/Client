...
public class ThueController implements MouseListener, ActionListener {
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        try {
            Thue thue = thueDAO.timKiem(maThue);
            ...
            if(tinhtrangnutthem == 0) {
                ...
                if(result == JOptionPane.YES_OPTION){
                    try {
                        ...
                        par.put("hieu_luc", 0);
                        Thue thue1 = thueDAO.timKiem(par).get(0);
                        thue1.setHieuLuc(false);
                        boolean check = thueDAO.capNhat(thue1);
                        if(!check){
                            JOptionPane.showMessageDialog(formThue, "Thêm thuế thất bại");
                            return;
                        }
                        thueDAO.them(thue);
