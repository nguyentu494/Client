...
public class NhanVienController implements ActionListener, MouseListener, KeyListener {
    ...
    public NhanVienController(FrmNhanVien nhanVienUI) {
        try {
            instance = AppUtils.getInstance();
            nhanVienDAO = instance.getNhanVienDAO();
        } catch (Exception e) {
        ...
        if(op.equals(nhanVienUI.getCbTkGioiTinh()) || op.equals(nhanVienUI.getCbTkChucVu()) || op.equals(nhanVienUI.getCbTkTinhTrang()) || op.equals(nhanVienUI.getCbTkCaLamViec())){
            if(!nhanVienUI.getCbTkGioiTinh().getSelectedItem().equals("--Giới tính--")){
                ...
                if(caLamViec.equals("CA_1")){
                    ...
                    conditions.put("caLamViec", CaLamViec.CA_2);
                }
                try {
                    dsLoc = nhanVienDAO.timKiem(conditions);
                } catch (Exception e) {
                ...
                Map<String, Object> conditions = new HashMap<>();
                String tinhTrang = nhanVienUI.getCbTkTinhTrang().getSelectedItem().toString();
                if(tinhTrang.equals("DANG_LAM")){
                    conditions.put("tinhTrang", TinhTrangNhanVien.DANG_LAM);
                } else if (tinhTrang.equals("TAM_NGHI")) {
                    conditions.put("tinhTrang", TinhTrangNhanVien.TAM_NGHI);
                } else{
                    conditions.put("tinhTrang", TinhTrangNhanVien.NGHI);
                }
                try {
                    dsLoc = nhanVienDAO.timKiem(conditions);
                } catch (Exception e) {
    ...
    public void loadComboBoxPhanTimKiem(){
        ...
        for (int i = 0; i < dsChucVu.length; i++) {
            if(dsChucVu[i].toString().equals("ADMIN")){
                continue;
            }
    ...
    public void mouseClicked(MouseEvent event) {
        if(trangThaiNutThemNV==1){
            ...
            NhanVien nvHienThuc = new NhanVien();
            try {
